        ...
        <?php while (have_posts()): the_post(); ?>
            ...
            <?php endif; ?>
                <main class="description-wrapper">
                    ...
                    <div class="meta-info">
                        <?php $field_camera_check = get_field('camera_brand');if ($field_camera_check) {echo '<div class="single-meta">';
                    $values_camera = get_field('camera_brand');if ($values_camera) {$field_camera = get_field_object('camera_brand');
                    echo '<p class="meta-text"><span class="meta-label">' . $field_camera['label'] . '</span>: ';foreach ($values_camera as $value_camera) {$tag_camera = get_tag($value_camera);
                    ...
                    echo '<p class="meta-text"><span class="meta-label">' . $field_lens['label'] . '</span>: ';foreach ($values_lens as $value_lens) {$tag_lens = get_tag($value_lens);
                    ...
                    echo '</p>';}
                    $values_film = get_field('film_used');if ($values_film) {$field_film = get_field_object('film_used');
                    echo '<p class="meta-text"><span class="meta-label">' . $field_film['label'] . '</span>: ';foreach ($values_film as $value_film) {$tag_film = get_tag($value_film);
